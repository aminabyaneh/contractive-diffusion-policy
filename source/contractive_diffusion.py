...
class BaseDiffusionSDE(DiffusionModel):
    """ Base class for Diffusion SDEs.
    This class provides the basic structure for training and sampling from diffusion SDEs.
    """
    ...
    def __init__(
        ...
        self.eigen_weight = eigen_weight
        self.lambda_contr = lambda_contr
        self.loss_type = loss_type
        self.jacobian_losses = None
    ...
    def add_noise(self, x0, t=None, eps=None):
        ...
        Args:
            x0 (torch.Tensor): Input data to which noise will be added.
            t (Optional[torch.Tensor]): Time step(s) for noise addition. If None, sampled randomly.
            eps (Optional[torch.Tensor]): Noise tensor. If None, sampled from standard normal.
    ...
    def loss(self, x0: torch.Tensor, condition: BaseNNCondition = None):
        """
        Compute the loss for the diffusion model.

        Args:
            x0 (torch.Tensor): Samples from the target distribution.
            condition (BaseNNCondition): Condition of x0. None indicates no condition.
        ...
        # compute the noise or data prediction loss
        ...
        self.jacobian_losses = compute_contractive_loss(self.model["diffusion"], xt, t, condition,
                                                        loss_type=self.loss_type, lambda_contr=self.lambda_contr)

        return (loss * self.loss_weight * (1 - self.fix_mask)).mean() + \
               self.jacobian_weight * self.jacobian_losses["jacobian_loss"] + \
               self.eigen_weight * self.jacobian_losses["eigen_max"]
    ...
    @property
    def contraction_losses(self):
        """ Get the contraction losses.

        Returns:
            dict: A dictionary containing the contraction losses.
                - "jacobian_loss": The Jacobian loss.
                - "eigen_loss": The Eigen loss.
                - "jacobian_norm": The Frobenius norm of the Jacobian.
                - "eigen_max": The maximum eigenvalue.
                - "eigen_avg": The average eigenvalue.
                - "eigen_std": The standard deviation of the eigenvalues.
        """

        return self.jacobian_losses
...
class DiscreteDiffusionSDE(BaseDiffusionSDE):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            epsilon, noise_schedule, noise_schedule_params, x_max, x_min, predict_noise, lambda_contr, loss_type,
            jacobian_weight, eigen_weight, device)
